...
class RCNNHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if not cfg.MODEL.SparseRCNN.USE_FPN:
            self.roi_head = Conv2d(2048, 256, kernel_size=1, padding=0, bias=True)
        else:
            self.roi_head = None
    ...
    def forward(self, features, bboxes, pro_features, pooler):
        ...
        # inst_interact.
        pro_features = pro_features.view(nr_boxes, N, self.d_model).permute(1, 0, 2)
        pro_features = pro_features.reshape(1, N * nr_boxes, self.d_model)
        ...
        return class_logits, pred_bboxes.view(N, nr_boxes, -1), obj_features
...
class DynamicConv(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.dynamic_layer = nn.Linear(self.hidden_dim, 2 * self.num_params)
